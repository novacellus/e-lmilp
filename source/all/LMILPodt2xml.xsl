...
<xsl:stylesheet xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:lmilp="http://scriptores.pl/"
    ...
    <!-- Pass2a: Ujęcie spacji w sąsiadujący tag -->
    <xsl:template match="text()[matches(.,'^(\s+|\s*\|\s*)$')]|text()[normalize-space(.) eq ',']|node()[normalize-space(.) eq '|\W']"
        mode="pass2a">
